...
import { Grid, Typography, TextField, IconButton, List, ListItem, ListItemAvatar, ListItemText, Avatar, Paper } from "@material-ui/core";
...
const ChatBox = (props) => {
  ...
  if (props.scope !== null && props.conversationId !== null) {
    ...
    getMessages(props.user._id).then((res) => setMessages(res));
    } else {
      setMessages([]);
    }
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    sendMessage(props.user._id, newMessage).then((res) => {
        setNewMessage("");
      });
